// app/services/réseaux-sociaux/page.tsx
"use client";
        ...
        <section className={cn(UI.max, "pt-12 pb-10")}>
          ...
          <Reveal delay={0.08}>
            <p className={cn("mt-5 max-w-3xl text-base md:text-lg", UI.subtle)}>
              On structure ta présence : hooks, formats, cadence, cohérence visuelle. Objectif : attirer, retenir, convertir.
              (Reels / TikTok / Shorts + versions adaptées.)
            </p>
          </Reveal>
          ...
          <Reveal delay={0.16}>
            <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              {[
                { k: "Format", v: "Reels / TikTok / Shorts" },
                { k: "Stratégie", v: "Hooks + angle" },
                { k: "Cohérence", v: "Style signature" },
                { k: "Résultat", v: "KPI + ajustements" },
              ].map((x) => (
        ...
        <section className={cn(UI.max, "pb-10")}>
          <Reveal>
            <div className={cn(UI.card, "p-7")}>
              <h2 className={UI.h2}>
                Contenu qui{" "}
                <span className="text-[#F5C518]">garde</span> l’attention
              </h2>
              <p className={cn("mt-2 max-w-3xl text-sm md:text-base", UI.subtle)}>
                On ne “poste pas”. On construit une machine : angles, série, répétition intelligente, et amélioration.
              </p>

              <div className="mt-6 grid gap-3 md:grid-cols-2">
                {[
                  "Plan mensuel (thèmes, séries, angles, hooks)",
                  "Tournage & montage short-form (rythme, cuts, sous-titres)",
                  "Templates visuels (typo, titres, codes couleur)",
                  "Exports optimisés (9:16) + variantes (1:1 / 16:9 si besoin)",
                  "A/B tests (hooks, durée, structure) + reporting KPI",
                  "Conseils publication (timing, CTA, captions, hashtags propres)",
                ].map((p) => (
        ...
        <section className={cn(UI.max, "pb-10")}>
          <Reveal>
            ...
            <p className={cn("mt-2 text-sm md:text-base max-w-3xl", UI.subtle)}>
              Trois niveaux. On ajuste selon cadence, tournage, nombre de formats et objectifs.
            </p>
          </Reveal>
            ...
            {[
              {
                ...
                hint: "Consistance",
                rec: false,
                d: "On lance une routine de contenu propre et régulière.",
                b: ["Plan simple", "Production formats courts", "Optimisation exports", "Suivi léger"],
              },
              {
                t: "Growth",
                hint: "Recommandé",
                rec: true,
                d: "On améliore régularité, qualité et résultats avec une vraie structure.",
                b: ["Plan mensuel structuré", "Hooks + montage + sous-titres", "Optimisation (timing/format)", "Suivi KPI"],
              },
              {
                t: "Full",
                hint: "Présence complète",
                rec: false,
                d: "Calendrier éditorial + production récurrente + tests + reporting.",
                b: ["Calendrier éditorial", "Production récurrente", "Optimisation + tests", "Reporting KPI"],
              },
            ].map((p, i) => (
              <motion.div
                key={p.t}
            ...
            <Reveal>
              <div className="max-w-2xl">
                ...
                <p className={cn("mt-3 text-sm md:text-base", UI.subtle)}>
                  Donne-nous ton objectif (clients, notoriété, bookings) + ta cible. On te sort un plan clair.
                </p>
              </div>
            </Reveal>
